// backend/migrations/YYYYMMDDHHMMSS-initial-schema-setup.mjs

export async function up(queryInterface, Sequelize) {
  console.log('INICIANDO MIGRAÇÃO CONSOLIDADA (ESM): Configuração inicial do esquema...');
  ...
  await queryInterface.createTable('CargosExercidos', {
    id: { allowNull: false, autoIncrement: true, primaryKey: true, type: Sequelize.INTEGER },
    nomeCargo: { type: Sequelize.ENUM("Venerável Mestre", "Primeiro Vigilante", "Segundo Vigilante", "Orador", "Orador Adjunto","Secretário", "Secretário Adjunto", "Chanceler", "Chanceler Adjunto", "Tesoureiro", "Tesoureiro Adjunto", "Mestre de Cerimônias", "Mestre de Harmonia", "Arquiteto", "Bibliotecário", "Mestre de Harmonia Adjunto", "Arquiteto Adjunto", "Bibliotecário Adjunto","Primeiro Diácono", "Segundo Diácono", "Primeiro Experto", "Segundo Experto", "Cobridor Interno", "Cobridor Externo", "Hospitaleiro", "Porta Bandeira", "Porta Estandarte", "Deputado Estadual", "Deputado Federal", "Sem cargo definido"), allowNull: false },
  ...
  // 8. Tabela Harmonia
  await queryInterface.createTable('Harmonia', {
    id: { allowNull: false, autoIncrement: true, primaryKey: true, type: Sequelize.INTEGER },
  ...
  console.log('MIGRAÇÃO CONSOLIDADA (ESM): Esquema inicial completo criado com sucesso.');
}

export async function down(queryInterface, Sequelize) {
  console.log('REVERTENDO MIGRAÇÃO CONSOLIDADA (ESM): Removendo esquema inicial...');
  ...
  console.log('REVERSÃO DA MIGRAÇÃO CONSOLIDADA (ESM) concluída.');
}